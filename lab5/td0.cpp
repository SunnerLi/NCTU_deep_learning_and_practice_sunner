...
class learning {
    public: 
        ...
        /**
         * 針對輸入的after state，考慮所有可能的popup情況，並用weighted-sum的方式加起來
         * 回傳的是E[V(S_{t+1})]
         */
        float expectation(const board& b) const {
            debug << "expectation " << std::endl << b;
            ...
            // 用一個陣列紀錄所有可能的結果盤面S_{t+1} (出現2和2)
            std::vector<board> possible_22_tile;
            for (int i = 0; i < space_num; i++) {
                for (int j = i+1; j < space_num; j++) {
                    board possible_b = b;
                    possible_b.set(space[i], 1);
                    possible_b.set(space[j], 1);
            ...
            // 用一個陣列紀錄所有可能的結果盤面S_{t+1} (出現2和4)
            std::vector<board> possible_24_tile;
            for (int i = 0; i < space_num; i++) {
                for (int j = 0; j < space_num; j++) {
                    if (i != j) {
                        board possible_b = b;
                        possible_b.set(space[i], 1);
                        possible_b.set(space[j], 2);
                        possible_24_tile.push_back(possible_b);
                        possible_b.set(space[i], 0);
                        possible_b.set(space[j], 0);
                    }
                }
            ...
            // 用一個陣列紀錄所有可能的結果盤面S_{t+1} (出現4和4)
            std::vector<board> possible_44_tile;
            for (int i = 0; i < space_num; i++) {
                for (int j = i+1; j < space_num; j++) {
                    if (i != j) {
                        board possible_b = b;
                        possible_b.set(space[i], 2);
                        possible_b.set(space[j], 2);
                        possible_44_tile.push_back(possible_b);
                        possible_b.set(space[i], 0);
                        possible_b.set(space[j], 0);
                    }
                }
            ...
            // 累加所有可能的value，並取weighted-sum，得到expectation of value
            float value = 0, value_22 = 0, value_24 = 0, value_44 = 0;
            for (feature* feat : feats) {
                for (board possible_b : possible_22_tile) {
                    value_22 += 0.81 * feat->estimate(possible_b);
                }
                for (board possible_b : possible_24_tile) {
                    value_24 += 0.09 * feat->estimate(possible_b);
                }
                for (board possible_b : possible_44_tile) {
                    value_44 += 0.01 * feat->estimate(possible_b);
                }
            }
            if (possible_22_tile.size() > 0)
                value_22 /= possible_22_tile.size();
            if (possible_24_tile.size() > 0)
                value_24 /= possible_24_tile.size();
            if (possible_44_tile.size() > 0)
                value_44 /= possible_44_tile.size();
            value =  value_22 + value_24 + value_44;
            return value;
        }
